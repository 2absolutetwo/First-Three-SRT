        ...
        {(() => {
          ...
          if (allSplit) {
            ...
            return (
              <div className="flex-1 min-h-0 flex flex-col overflow-hidden p-4 gap-3">
                <div className="grid gap-4 shrink-0" style={{ gridTemplateColumns: `repeat(${langs.length}, minmax(0, 1fr))` }}>
                  {langs.map((lang, idx) => (
                ...
                <div className="flex-1 min-h-0 overflow-y-auto pr-1">
                  <div className="grid gap-4 auto-rows-min" style={{ gridTemplateColumns: `repeat(${langs.length}, minmax(0, 1fr))` }}>
          ...
          return (
            <div className="flex-1 min-h-0 flex overflow-hidden p-4 gap-4">
              {langs.map((lang, idx) => (
                <div key={idx} className="flex-1 min-h-0 flex flex-col rounded-xl border border-border bg-card shadow-sm overflow-hidden min-w-0">
                  {renderLangHeader(lang, idx)}
                  {showFind[`${activeId}:${idx}`] && (
